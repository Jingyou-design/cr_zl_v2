...
async def _eval_one(index: int, solution: str) -> dict:
    ...
    eval_prompt = f"""请对以下技术方案进行**完整的**专利审查检索和新颖性/创造性评估。
                请使用 patenthub 技能进行现有技术检索，使用专利审查检索技能评估 X/Y 类文献。

                待评估方案：
                {solution}

                **请输出完整的评估报告，必须详细展开以下所有部分，不允许省略或只给概述：**

                1) **检索策略**：详细说明检索的关键词、IPC分类号、数据库和检索范围；
                2) **对比文件列表**：逐一列出所有相关的对比文件，包含专利号、标题、摘要，并详细说明每篇对比文件与本案技术特征的具体对比分析；
                3) **新颖性结论**：逐条分析本案的每项技术特征是否已被对比文件单独公开，给出详细推理过程；
                4) **创造性结论**：详细分析本案技术方案对本领域技术人员来说是否显而易见，结合对比文件说明组合启示；
                5) **改进建议**（如评估不通过）：给出具体、可操作的改进方向。

                **最后请明确给出：【评估结果：通过 / 不通过】**

                注意：必须输出完整的详细报告，不能只给出结果概述。每个部分都要充分展开论述。
                """
